...
def get_apt_trade_data(lawd_cd: str, deal_ymd: str, ctx: Optional[Any] = None) -> TextContent:
    ...
    """
    def call(context):
        from mcp_kr_realestate.apis.apt_trade import get_apt_trade_data as api_func
        return api_func(lawd_cd, deal_ymd)
...
@mcp.tool(
    ...
    description="""
이 도구를 사용하기 전에 반드시 먼저 법정동 코드(5자리, lawd_cd)를 조회하고 선택해야 합니다. 법정동 코드는 단독/다가구 실거래가 데이터를 조회하는 데 필수입니다.
